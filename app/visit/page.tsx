    ...
    <main style={{ minHeight: '100vh', padding: 'clamp(6rem, 10vw, 8rem) clamp(1.5rem, 5vw, 4rem) clamp(3rem, 6vw, 5rem)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 340px), 1fr))', gap: '3rem 6rem', maxWidth: '900px' }}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15, duration: 0.45 }}>
          <h2 style={{ fontFamily: 'var(--font-syne-var), sans-serif', fontWeight: 700, fontSize: '0.6rem', letterSpacing: '0.24em', textTransform: 'uppercase', color: '#A8B5A2', marginBottom: '1rem' }}>
            Address
          </h2>
          <p style={{ fontFamily: 'var(--font-syne-var), sans-serif', fontWeight: 700, fontSize: 'clamp(1.4rem, 3vw, 2rem)', lineHeight: 1.3, color: '#2C2C2C', marginBottom: '1.25rem' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25, duration: 0.45 }}>
          ...
          {HOURS.map(({ day, hours }) => (
            <div key={day} style={{ display: 'flex', justifyContent: 'space-between', gap: '2rem', padding: '0.85rem 0', borderBottom: '1px solid #EDE8DF' }}>
              <span style={{ fontFamily: 'var(--font-syne-var), sans-serif', fontWeight: 700, fontSize: '0.95rem', color: '#2C2C2C' }}>{day}</span>
              <span style={{ fontFamily: 'var(--font-inter-var), sans-serif', fontWeight: 300, fontSize: '0.95rem', color: '#2C2C2C' }}>{hours}</span>
            </div>
          ))}
          <p style={{ marginTop: '1.25rem', fontFamily: 'var(--font-inter-var), sans-serif', fontWeight: 300, fontSize: '0.82rem', color: '#2C2C2C', opacity: 0.5 }}>
